...
class Photo(Resource):
    ...
    def get(self, hydrant_id) -> Response:
        ...
        if not isinstance(hydrant_id, int) or hydrant_id < 0:
            return create_response(message={"error": "hydrant id must be positive integer"}, status_code=STATUS_CODES["bad_request"])

        # Check that hydrant exists
        hydrant = fetchone_query("SELECT id FROM hydrants WHERE id = %s", (hydrant_id,))
        if not hydrant:
            return create_response(message={"error": "hydrant not found"}, status_code=STATUS_CODES["not_found"])
        
        # Get the data
        photos = fetchall_query("SELECT posizione, data FROM photos WHERE idI = %s", (hydrant_id,))

        # Check if photos exist
        if not photos:
            return create_response(message={"error": "no photos found"}, status_code=STATUS_CODES["not_found"])
    ...
    def post(self) -> Response:
        ...
        if not (isinstance(hydrant_id, int) and hydrant_id >= 0 or isinstance(hydrant_id, str) and hydrant_id.isdigit()):
            return create_response(message={'error': 'hydrant id must be positive integer or numeric string'}, status_code=STATUS_CODES["bad_request"])
        if not isinstance(position, str):
            return create_response(message={'error': 'position must be string value.'}, status_code=STATUS_CODES["bad_request"])
        if not isinstance(date, str):
            return create_response(message={'error': 'date must be string value.'}, status_code=STATUS_CODES["bad_request"])
        ...
        existing_photo = fetchone_query("SELECT * FROM photos WHERE idI = %s AND posizione = %s AND data = %s", (hydrant_id, position, date))
        if existing_photo:
            return create_response(message={'error': 'photo already exists.'}, status_code=STATUS_CODES["bad_request"])
        ...
        # Return the response
        return create_response(message={'outcome': 'photo successfully created',
                                        "location": f"http://{API_SERVER_HOST}:{API_SERVER_PORT}/{Photo.ENDPOINT_PATHS[0]}/{lastrowid}"}, status_code=STATUS_CODES["created"])
